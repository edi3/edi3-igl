...
|--|-----------|--------------|
|R1|Any Customs Authority SHOULD be able to participate in the ICL by running a Node| |
|R2|Participating in the network by running a node SHOULD NOT automatically grant access to the content of a document| |
|R3|A message on the ICL SHOULD provide a mechanism for discovering the full content of the document to which it relates| |
